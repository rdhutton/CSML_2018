...
class IndexedString(object):
    ...
    def __init__(self, raw_string, split_expression=r'\W+', bow=True,ngram_size=1,stop_words=True):
        """Initializer.
        ...
        if ngram_size == 1 :
            for i, word in enumerate(self.as_np):
                if word in non_vocab:
                    continue
                # RH include class of single character words as "non-words" / optionally stop words too
                # if non_word(word):
                if non_word(word) or (len(word) < 2) or (
                        (stop_words == False) and (word in stopwords.words('english'))):
                    # print("non word added ",word )
                    non_vocab.add(word)
                    continue
                if bow:
                ...
                if n < ngram_size:
                    if word in non_vocab:
                        ...
                        if n == 0:
                            ...
                            ngram += ' ' + word
                        n+=1
                # if ngram complete add in same way as adding word in single word case above
                if n >= ngram_size:    
                    if bow:
                        if ngram not in vocab:
                            vocab[ngram] = len(vocab)
                            self.inverse_vocab.append(ngram)
                            self.positions.append([])
                        idx_word = vocab[ngram]
                        self.positions[idx_word].append(ngram_position)
                    else:
                        self.inverse_vocab.append(word)
                        self.positions.append(ngram_position)
                    n = 0
                    ngram = ""
            if not bow:
                self.positions = np.array(self.positions)
    ...
    def inverse_removing(self, words_to_remove):
        ...
        if ngram length > 1, treats words_to_remove as first word in each ngram
        ...
        """
        ngrams_to_remove = words_to_remove
        mask = np.ones(self.as_np.shape[0], dtype='bool') 

        # add appropriate number of words to first word of ngram
        words_to_remove_idxs =  self.__get_idxs(ngrams_to_remove)
        additional_idxs = []
        for n in range(2,self.ngram_size+1):
            #(n-1)*2 adds every alternate token (the words not the non-words. )
            additional_idxs += ([x + (n-1)*2 for x in words_to_remove_idxs])
        words_to_remove_idxs += additional_idxs   

        mask[words_to_remove_idxs] = False
        if not self.bow:
...
#  RH add class
class IndexedSentences(IndexedCharacters):
    ...
    def __init__(self, raw_string, split_expression=r'\W+', bow=True,ngram_size=1,stop_words=True):
        ...
        # self.as_list = re.split(r'(%s)|$' % split_expression, self.raw)
        self.as_list = re.split(r'([.!?]+)|$', self.raw)
        self.as_np = np.array(self.as_list)
        non_word = re.compile(r'([.!?]+)|$').match
        self.string_start = np.hstack(
            ([0], np.cumsum([len(x) for x in self.as_np[:-1]])))
        ...
        for i, word in enumerate(self.as_np):
            if word in non_vocab:
                continue
            if non_word(word) :      # matches regex for non-word
                # print("non word added ",word )
                non_vocab.add(word)
                continue
            if bow:
...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
        ...
        # RH add option to use IndexedSentences
        if self.sentence_level:
            indexed_string = IndexedSentences(text_instance, bow=self.bow)
        else:
            indexed_string = IndexedCharacters(
                ...
                ngram_size=self.ngram_size,stop_words=self.stop_words)
        ...
        # RH - set new field
        ret_exp.classifier_err = self.classifier_error
    ...
    def __data_labels_distances(self,
        ...
        # temp change - for sentences
        # sample = self.random_state.randint(int(0.1*doc_size)+1, int(0.4*doc_size) + 1, num_samples - 1)
        ...
        # check for return value from classifier indicating a problem
        if 0 in labels :
            self.classifier_error = True
        # debug
        # print(inverse_data)
